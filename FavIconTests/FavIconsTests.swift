...
class FavIconTests: XCTestCase {
    ...
    func testDownloading() {
        ...
        // Test that results have been added into the array
        XCTAssertTrue(actualResults.count != 0)
        // Test that the favicons height & width are equal.
    ...
    func testChooseIcon() {
        guard let url = URL(string: "https://google.com/favicon.ico") else {
            XCTFail("Invalid URL.")
            return
        }
        let mixedIcons = [
            DetectedIcon(url: url, type: .shortcut, width: 32, height: 32),
            DetectedIcon(url: url, type: .classic, width: 64, height: 64),
            DetectedIcon(url: url, type: .appleIOSWebClip, width: 64, height: 64),
            DetectedIcon(url: url, type: .appleOSXSafariTab, width: 144, height: 144),
            DetectedIcon(url: url, type: .classic)
        ]
        let noSizeIcons = [
